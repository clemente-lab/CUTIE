...
def assign_statistics(samp_var1, samp_var2, statistic, pearson_stats, 
    spearman_stats, mine_stats, mine_bins, pvalue_bins, paired, f1type, log_fp):
    """
...
def pointwise_comparison(samp_var1, samp_var2, pvalues, corrs, working_dir, 
    ...
    if statistic == 'kpc':
        infln_metrics = ['cookd', 'dffits', 'cutie_1pc'] # 'dsr'
        infln_mapping = {
        ...
        infln_metrics = ['cookd', 'dffits', 'cutie_1sc'] # 'dsr'
        infln_mapping = {
    ...
    regions, regions_set = calculate_intersection(infln_metrics, 
...
def calculate_intersection(names, sets, log_fp):
    ...
    OUTPUTS
    regions     - List of strings. Each string describes a region (union) in the
                  Venn Diagram.
...
def get_initial_corr(n_var1, n_var2, pvalues, threshold, paired, log_fp):
    """
...
def updatek_cutie(initial_corr, pvalues, samp_var1, samp_var2, threshold, 
    ...
    (true_corr, true_comb_to_rev, false_comb_to_rev, corr_extrema_p, 
        ...
        var2_counter) = cutiek_true_corr(initial_corr, samp_var1,
        samp_var2, pvalues, corrs, threshold, paired, statistic, forward_stats, 
        reverse_stats, resample_k, true_corr, true_comb_to_rev, 
        false_comb_to_rev, extrema_p, extrema_r, fold, fold_value, n_replicates,
    ...
    return (true_corr, true_comb_to_rev, false_comb_to_rev, extrema_p, extrema_r,
        samp_counter, var1_counter, var2_counter)
...
def evaluate_correlation_k(var1, var2, n_samp, samp_var1, samp_var2, pvalues, 
    ...
    """     
    # CUtIe
    if statistic in ['kpc', 'rpc', 'ksc', 'rsc', 'mine', 'rmine']:
    ...
    # jackknife
    elif statistic in ['jkp', 'rjkp', 'jks', 'rjks', 'jkm', 'rjkm']:
    ...
    # bootstrap
    elif statistic in ['bsp', 'rbsp', 'bss', 'rbss', 'bsm' 'rbsm']:
...
def jackknifek_cutie(var1, var2, n_samp, samp_var1, samp_var2, pvalues, 
    ...
    if forward is True:
        exceeds = test_CI(CI, threshold, exceeds, 
            [item for sublist in combs for item in sublist], upper = True, 
            CI_method)
    elif forward is False:
        exceeds = test_CI(CI, threshold, exceeds, 
            [item for sublist in combs for item in sublist], upper = False, 
            CI_method)

    return reverse, exceeds, extrema_r, extrema_p


def bootstrap_cutie(var1, var2, n_samp, samp_var1, samp_var2, pvalues, 
    ...
    if forward is True:
        exceeds = test_CI(CI, threshold, exceeds, xrange(n_samp), upper = True,
            CI_method)
    elif forward is False:
        exceeds = test_CI(CI, threshold, exceeds, xrange(n_samp), upper = False,
            CI_method)  
...
def lof_fit(samp_var1, samp_var2, n_samp, working_dir, paired, log_fp):
    ...
    if not paired:
        ...
        df = df.join(df2, lsuffix='_df1r', rsuffix='_df2') 
    # drop NA
